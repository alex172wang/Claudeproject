...
使用方法:
    python manage.py shell < scripts/init_pools.py
或:
    python manage.py runscript init_pools (如果安装了django-extensions)
"""
...
def init_etfs():
    ...
    for etf_data in ETF_DEFINITIONS:
        try:
            ...
            if created:
                created_count += 1
                print(f"  创建ETF: {etf.code} - {etf.name}")
            else:
                updated_count += 1
                print(f"  更新ETF: {etf.code} - {etf.name}")
        except Exception as e:
            print(f"  错误处理ETF {etf_data.get('code')}: {e}")

    print(f"\nETF初始化完成: 创建 {created_count} 条, 更新 {updated_count} 条")
        ...
        try:
            ...
            for member_data in pool_data.get('members', []):
                try:
                    ...
                    member_count += 1
                    if member_created:
                        print(f"    + 添加成员: {etf.code} - {etf.name}")
                except ETF.DoesNotExist:
                    print(f"    ! ETF不存在: {member_data['code']}")
                except Exception as e:
                    print(f"    ! 添加成员失败 {member_data['code']}: {e}")

        except Exception as e:
            print(f"  错误处理品种池 {pool_data.get('code')}: {e}")

    print(f"\n品种池初始化完成: 创建 {created_count} 个池, {member_count} 个成员")
    return created_count, member_count
...
def main():
    ...
    # 初始化ETF
    etf_created, etf_updated = init_etfs()

    # 初始化品种池
    pool_created, member_count = init_pools()

    print("\n" + "=" * 60)
    print("初始化完成汇总")
    print("=" * 60)
    print(f"ETF: 创建 {etf_created} 条, 更新 {etf_updated} 条")
    print(f"品种池: 创建 {pool_created} 个")
    print(f"池成员: 创建 {member_count} 个")
    print("=" * 60)
